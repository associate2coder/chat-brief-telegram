...
- **Tests:** unit tests live beside the module they test, as `*.test.ts` (e.g. `src/http/app.test.ts`). Integration-style tests that fake the Telegram API belong in `src/relay/` once that layer exists. Run with `npm test` (Vitest).
...
This is the scaffolded skeleton (`/sdd:scaffold`) — `src/http/app.ts` exposes only a `/health`
check. The actual relay endpoint (shared-secret check, summary validation, Telegram delivery,
in-conversation failure reporting) is built by the `chat-brief-telegram` feature — see
`docs/features/chat-brief-telegram/` and `docs/roadmap.md`.
